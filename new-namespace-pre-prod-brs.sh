...
# -Run ./new-namespace-pre-prod-brs.sh <new-ns-name> pre-prod                           #
#                                                                                       #
# NOTE!                                                                                 #
# The name supplied in <new-ns-name> will be the default namespace name, appended       #
# with 'brs'. This will also be the default hostname. Please modify the hostname if     #
# you prefer a different one.                                                           #
#                                                                                       #
# Expected Outputs:                                                                     #
#   namespace: <new-ns-name>-brs                                                        #
#   host: la-ecom-<new-ns-name>.inchcapedigital.com                                     #
#########################################################################################

NEW_NS="brazil-south-$1"
REF_NS="brazil-south-$2"
...
# NEW ENV FOLDER
cd $NEW_NS
REF_HOST="la-ecom-preprod.inchcapedigital.com"
NEW_HOST="la-ecom-$1.inchcapedigital.com"
#Applications
...
for app in "x15-platform-api" "psql-client" "finance-gateway-api" "configuration-management"; do
    ...
    if [[ "$app" == "x15-platform-api" ]]; then
        REF_APPCONFIG="http://configuration-management-$2-brs:8086"
        NEW_APPCONFIG="http://configuration-management-$1-brs:8086"
