...
# -Run ./new-namespace-pre-prod-au.sh <new-ns-name> pre-prod                            #
#                                                                                       #
# NOTE!                                                                                 #
...
cp -v $REF_NS/markets/"cms-mock-$2-au-au-sub-dis.yml" $NEW_NS/markets/"cms-mock-$1-au-au-sub-dis.yml"
cp -v $REF_NS/markets/"dxp-widgets-$2-au-au-sub-dis.yml" $NEW_NS/markets/"dxp-widgets-$1-au-au-sub-dis.yml"
cp -v $REF_NS/markets/"widgets-$2-au-au-sub-dis.yml" $NEW_NS/markets/"widgets-$1-au-au-sub-dis.yml"
CMS="cms-mock-$1-au-au-sub-dis.yml"
DXP="dxp-widgets-$1-au-au-sub-dis.yml"
WIDGETS="widgets-$1-au-au-sub-dis.yml"
...
# NEW ENV FOLDER
cd $NEW_NS
REF_HOST="au-ecom-preprod.inchcapedigital.com"
...
for app in "cms-mock" "dxp-widgets" "widgets"; do
    ...
    METADATA_NAME="${app}-$1-au-au-sub-dis"
    yq e ".metadata.name = \"$METADATA_NAME\"" -i "$FILE"
    yq e ".spec.destination.namespace = \"$NAMESPACE\"" -i "$FILE"
    yq e ".spec.source.helm.values |= sub(\"$REF_HOST\", \"$NEW_HOST\")" -i "$FILE"

    if [[ "$app" == "cms-mock" ]]; then
        REF_AUTH_SECRET="cms-mock-$2-au-au-sub-dis"
